...
class NCCP_Chemotherapy_Database:
    ...
    def tabulate_indications(self):
        ...
        for ind in self.indications.values():
            entry = [ind.code, ind.description.replace("\n", " "), ", ".join(ind.diseases),
...
def main(harmonization_file=None, output=None):
    ...
    if output:
        data.tabulate_indications().to_csv(output, sep="\t", index=True)
    return data


def _setup_argparser():
    argparser = argparse.ArgumentParser()
    argparser.add_argument("-x", "--harmonization-file",
                           help="TSV file where column 1 contains original regimen "
                                "names, and column 2 contains replacement names to "
                                "be used instead.")
    argparser.add_argument("-o", "--output",
                           help="Path to write a TSV file of all parser NCCP "
                                "indications and associated information.")
    return argparser
